...
export const SellingNotification = ({getNotificationCount}) => {
  ...
  const getNotification = async (page, size) => {
    try {
      const res = await ProductServices.getNotification(user_id, 'selling', page, size);
          ...
          <div className="row">
            {notification?.notifications?.length > 0 ?
              (notification?.notifications?.map((data, index) => {
                return (
                  <div className="col-lg-6 col-md-6 col-12" style={{ cursor: 'pointer' }} key={data?.id} onClick={() => { navigate(`/my-seller-account?tab=order-management`) }}>
                    <div className="n-l-d">
              ...
              :
              <NoDataFound title={'No selling notification found'} />
            }
          </div>
