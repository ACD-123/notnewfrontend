            ...
            <div className="px-2">
                {isLoggedin ? (
                    <>
                        ...
                        >
                            <h3>{data?.product?.name?.substring(0, 20)}...</h3>
                            {data?.product?.auctioned ?
                            ...
                            <h4>{data?.product?.description?.substring(0, 15)}...</h4>
                        </Link>
                    </>
                ) : (
                    <>
                        <Link to="/signin">
                            <h3>{data?.product?.name?.substring(0, 50)}...</h3>
                            {data?.product?.auctioned ?
                                <h2>${data?.product?.bids}</h2>
                                :
                                <h2>${data?.product?.price}
                            ...
                            <h4>{data?.product?.description?.substring(0, 50)}...</h4>
                        </Link>
                    </>
                )}
                {isLoggedin ? (
        ...
        <div className="px-2">
            {isLoggedin ? (
                <>
                    ...
                    >
                        <h3>{data?.name?.substring(0, 20)}...</h3>
                        {data?.auctioned ?
                            <h2>${data?.bids}</h2>
                            :
                            <h2>${data?.price}
                        ...
                        <h4>{data?.description?.substring(0, 15)}...</h4>
                    </Link>
                </>
            ) : (
                <>
                    <Link to="/signin">
                        <h3>{data?.name?.substring(0, 50)}...</h3>
                        {data?.auctioned ?
                            <h2>${data?.bids}</h2>
                            :
                            <h2>${data?.price}
                        ...
                        <h4>{data?.description?.substring(0, 50)}...</h4>
                    </Link>
                </>
            )}
            {isLoggedin ? (
